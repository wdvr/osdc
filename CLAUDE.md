...
- [ ] **Warm full-GPU (1-GPU) pods + evict-on-demand** (approved): keep 1–2 warm *full* H100/B200 pods so single-GPU claims (and the test-repro path) are instant. REQUIRES the reconciler/claim path to **evict** warm 1-GPU pods when a 2/4/8-GPU request needs their node (NVLINK contiguity) — ~10–30s eviction (pod terminate + device-plugin reconcile), acceptable. Pack warm 1-GPU pods onto as few nodes as possible to keep full nodes free. DO NOT bump `WARM_POOL_TARGETS` for full GPU until the eviction logic exists, or multi-GPU requests will be blocked.
