...
def store_alb_mapping(
    ...
    """
    Store ALB mapping in PostgreSQL for cleanup (Jupyter only, SSH uses proxy)
    ...
    try:
        ...
        with get_db_cursor() as cur:
            cur.execute("""
                INSERT INTO alb_target_groups (
                    reservation_id, domain_name, jupyter_target_group_arn, 
                    jupyter_rule_arn, expires_at
                )
                VALUES (%s, %s, %s, %s, %s)
                ON CONFLICT (reservation_id) 
                DO UPDATE SET 
                    domain_name = EXCLUDED.domain_name,
                    jupyter_target_group_arn = EXCLUDED.jupyter_target_group_arn,
                    jupyter_rule_arn = EXCLUDED.jupyter_rule_arn,
                    expires_at = EXCLUDED.expires_at
            """, (reservation_id, domain_name, jupyter_target_group_arn, 
                  jupyter_rule_arn, expires_at_dt))
...
def delete_alb_mapping(reservation_id: str) -> bool:
    ...
    This function is optimized to minimize database connection hold time:
    1. Quick query to get mapping data (releases connection immediately)
    2. AWS API calls without holding database connection
    3. Quick delete of database record
    ...
    """
    try:
        # STEP 1: Get mapping data from database (quick, releases connection immediately)
        mapping = None
        with get_db_cursor(readonly=True) as cur:
            cur.execute("""
                SELECT jupyter_rule_arn, jupyter_target_group_arn
                FROM alb_target_groups
                WHERE reservation_id = %s
            """, (reservation_id,))
            mapping = cur.fetchone()
        
        # Connection is now returned to pool - ready for other operations
        
        if not mapping:
            logger.warning(f"No ALB mapping found for reservation {reservation_id}")
            return True
        
        # STEP 2: Delete AWS resources (NO database connection held during this)
        ...
        # Wait for rule deletion to propagate (no connection held during sleep)
        time.sleep(2)
        ...
        # STEP 3: Delete database record (quick, separate transaction)
        with get_db_cursor() as cur:
            cur.execute("""
                DELETE FROM alb_target_groups
                WHERE reservation_id = %s
            """, (reservation_id,))
