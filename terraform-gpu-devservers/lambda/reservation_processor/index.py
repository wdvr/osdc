...
def is_foreign_snapshot(snapshot: dict[str, Any]) -> bool:
    """Whether a snapshot was produced by something other than gpu-dev.

    `CopySnapshot` copies the source snapshot's tags onto the copy, so
    `tag:gpu-dev-user` does not prove we authored a snapshot -- a security
    scanner copying a user's disk yields one tagged as ours. Such copies must
    never gate a reservation (they can stay pending for hours) nor be used as a
    restore source (they hold a point-in-time older than their StartTime, so
    they can win `max(StartTime)` and silently restore stale data).

    Matched on copy markers rather than a required gpu-dev tag: ~2% of real
    snapshots are hand-made recovery ones carrying no SnapshotType tag.
    """
    tags = {tag["Key"]: tag["Value"] for tag in snapshot.get("Tags", [])}
    if any(key.lower().startswith("wiz") for key in tags):
        return True
    # A copy has no source volume in this account.
    if snapshot.get("VolumeId") == "vol-ffffffff":
        return True
    return "auto-generated by Wiz" in (snapshot.get("Description") or "")
        ...
        if not instances:
            ...
            try:
                ...
                if no_capacity_count > 0:
                    ...
                    return f"No {gpu_type.upper()} spot capacity in {az_list} — retrying every ~60s"
...
def allocate_gpu_resources(reservation_id: str, request: dict[str, Any], trace_data: dict = None) -> None:
    ...
    try:
        ...
        # Collect DNS CNAME result (was running in parallel with pod creation + ready wait)
        if dns_future:
            dns_future.result()
        ...
        # Store domain mapping now that we have node IPs and port
        if domain_name:
            ...
            stored = store_domain_mapping(
                ...
                node_port, reservation_id, expires_ts)
            if not stored:
                logger.warning(f"Domain mapping collision for {domain_name}, retrying")
                for _ in range(5):
                    domain_name = generate_unique_name()
            ...
            dns_result["domain_name"] = domain_name
            logger.info(f"Domain {domain_name} mapped for reservation {reservation_id}")
...
def create_disk_from_snapshot_or_empty(user_id: str, availability_zone: str, disk_name: str = None, reservation_id: str = None, skip_snapshot_restore: bool = False) -> tuple[str, bool, str]:
    ...
    try:
        ...
        # Completed first: a pending snapshot only matters if it is newer than
        # the newest restore point we already have.
        snapshot_filters = [
            {"Name": "tag:gpu-dev-user", "Values": [user_id]},
        ...
        for snap in snapshots:
            ...
            if 'delete-date' not in tags and not is_foreign_snapshot(snap):
                active_snapshots.append(snap)

        latest_snapshot = max(active_snapshots, key=lambda s: s['StartTime']) if active_snapshots else None

        # Now check for a pending snapshot of our own (from a recent expiry)
        pending_filters = [
            {"Name": "tag:gpu-dev-user", "Values": [user_id]},
        ...
        pending_snapshots = [
            snap for snap in pending_response.get('Snapshots', [])
            if not is_foreign_snapshot(snap)
        ]
        if pending_snapshots:
            latest_pending = max(pending_snapshots, key=lambda s: s['StartTime'])
            snapshot_id = latest_pending['SnapshotId']
            if latest_snapshot and latest_pending['StartTime'] <= latest_snapshot['StartTime']:
                logger.info(
                    f"Ignoring pending snapshot {snapshot_id} - older than completed "
                    f"{latest_snapshot['SnapshotId']}"
                )
            else:
