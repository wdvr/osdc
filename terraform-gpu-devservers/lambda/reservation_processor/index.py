...
def allocate_gpu_resources(reservation_id: str, request: dict[str, Any]) -> None:
    ...
    try:
        ...
        # Generate Jupyter URL - default to IP+port, ALB setup will override to HTTPS if successful
        # NOTE: Never use http://domain:port - domain points to ALB which only has 443/80
        # If ALB setup fails, we need IP+port as fallback (NodePort on EC2 node)
    ...
    try:
        ...
        if "Jupyter Lab started successfully" in exec_resp:
            ...
            try:
                ...
                # Build Jupyter URL - use IP+port since ALB isn't set up for late-enabled Jupyter
                # NOTE: http://domain:port doesn't work - domain points to ALB which only has 443/80
                # TODO: Add ALB setup for late-enabled Jupyter to get HTTPS support
