"""
GPU Reservation Processor
Handles reservation requests and manages K8s pod allocation
(Migrated to PostgreSQL/PGMQ - formerly Lambda)
"""
...
# This processor uses the hardcoded config for pod resource allocation.
# 
...
def find_reservation_by_prefix(reservation_id: str, user_id: str = None) -> dict:
    """Find reservation by ID prefix with optional user validation - uses PostgreSQL LIKE"""
    try:
        ...
        with get_db_cursor() as cur:
            if user_id:
                ...
                cur.execute("""
                    ...
                    WHERE user_id = %s AND reservation_id LIKE %s
                    ORDER BY created_at DESC
                """, (user_id, f"{reservation_id}%"))
            else:
                # Fallback - search all reservations by prefix
                cur.execute("""
                    SELECT * FROM reservations
        ...
        for record in event.get("Records", []):
            if record.get("eventSource") == "aws:sqs":
                ...
                try:
                    ...
                    if not skip_version_check:
                        try:
                            ...
                            if reservation_id:
                                ...
                                # Message deletion handled by main.py
                            else:
                                logger.error(
                    ...
                    # Message deletion handled by main.py (PGMQ ack)
...
# scan_dynamodb_paginated removed - replaced with PostgreSQL list functions
# Old DynamoDB pagination helper no longer needed with PostgreSQL
        ...
        if reservation_id:
            try:
                ...
                initial_record = {
                    ...
                    "created_at": reservation_request.get("created_at", datetime.now(UTC).isoformat()),
                    "status": "pending",
                ...
                initial_record["lambda_version"] = PROCESSOR_VERSION
...
def check_all_multinode_nodes_ready(master_reservation_id: str, total_nodes: int) -> bool:
    ...
    try:
        # Get all nodes in the multinode reservation (including master)
        nodes = list_multinode_reservations(master_reservation_id)
        logger.info(
...
def coordinate_multinode_reservation(master_reservation_id: str, total_nodes: int) -> bool:
    ...
    try:
        ...
        all_nodes = list_multinode_reservations(master_reservation_id)
        
        # Filter for nodes in pending status
        nodes = [node for node in all_nodes if node.get('status') == 'pending']
...
def process_multinode_individual_node(message_body: dict) -> bool:
    ...
    try:
        ...
        node_data = get_reservation(reservation_id)
...
def update_all_multinode_status(master_reservation_id: str, status: str, failure_reason: str = None):
    ...
    try:
        # Get all nodes in the multinode reservation
        nodes = list_multinode_reservations(master_reservation_id)
...
def calculate_multinode_queue_position_and_wait_time(master_reservation_id: str, total_gpus_needed: int, gpu_type: str, available_gpus: int) -> dict:
    ...
    try:
        ...
        all_queued = list_reservations_by_status("queued")
        queued_reservations = [r for r in all_queued if r.get('gpu_type') == gpu_type]
        ...
        if total_gpus_ahead > 0:
            ...
            try:
                all_active = list_reservations_by_status("active")
                active_reservations = [r for r in all_active if r.get('gpu_type') == gpu_type]
                    ...
                    if expires_at:
                        try:
                            ...
                            minutes_until_expiry = int(
                                (expire_time - datetime.now(UTC)).total_seconds() / 60)
...
def update_gpu_availability_table(
    ...
    try:
        ...
        # Note: Availability table updates are out of scope for this migration
        # This table is not yet migrated to PostgreSQL
        # TODO: Migrate availability tracking to PostgreSQL
        logger.warning(f"Skipping availability table update for {gpu_type} (not yet migrated)")
        ...
        # Don't raise since this is not critical
...
# delete_sqs_message function removed - message deletion now handled by main.py using PGMQ
...
def append_status_history_local(reservation_id: str, timestamp: str, message: str) -> None:
    """Local wrapper for append_status_history that matches the old signature"""
    try:
        new_entry = {
            "timestamp": timestamp,
            "message": message
        ...
        # Use the shared append_status_history function
        from shared.reservation_db import append_status_history as append_status_history_shared
        success = append_status_history_shared(reservation_id, new_entry)
        
        if success:
            logger.debug(
                f"Appended status history entry for {reservation_id}: {message}")
        else:
            logger.error(f"Failed to append status history for {reservation_id}")
...
def update_reservation_fields(reservation_id: str, **fields) -> None:
    ...
    try:
        ...
        # Use the shared update_reservation function
...
def mark_disk_in_use(user_id: str, disk_name: str, in_use: bool, reservation_id: str = None) -> None:
    ...
    try:
        ...
        # Build update dict for PostgreSQL
        updates = {
            'in_use': in_use,
            'last_used': now,
        }
...
def should_use_persistent_disk(user_id: str, current_reservation_id: str) -> bool:
    ...
    try:
        ...
        # Use PostgreSQL to query for active reservations
        # Note: This would need a proper implementation with list_reservations_by_user
        # For now, return True to allow persistent disks
        logger.warning("should_use_persistent_disk check not fully migrated - defaulting to True")
        return True
    ...
    try:
        ...
        if domain_name:
            try:
                ...
                # Store domain mapping in PostgreSQL
                from shared.dns_utils import store_domain_mapping
                store_domain_mapping(
                    subdomain=domain_name,  # Use short name, not full FQDN
                    target_ip=target_ip,  # Use private IP for VPC-internal access
...
def calculate_queue_position_and_wait_time(
    ...
    try:
        ...
        active_reservations = list_reservations_by_status("active")

        # Get all queued/pending reservations for this GPU type
        queued_reservations = []
        for status in ["queued", "pending"]:
            status_reservations = list_reservations_by_status(status)
            # Filter by GPU type
    ...
    try:
        ...
        try:
            ...
            if e.status == 404:
                ...
                try:
                    current_reservation = get_reservation(reservation_id) or {}
                ...
                for event in sorted_events[:5]:
                    ...
                    if event.reason == "FailedScheduling":
                            ...
                            if oldest_ts:
                                ...
                                if time_stuck > 30:
                                    ...
                                    try:
                                        ...
                                        res_item = get_reservation(reservation_id) or {}
                                        requested_gpus = int(
                        ...
                        if "Insufficient nvidia.com/gpu" in event.message:
                            ...
                            try:
                                res_item = get_reservation(reservation_id)
                                if res_item is None:
                                    res_item = {}
                                gpu_type = res_item.get("gpu_type", "")
...
def process_scheduled_queue_management():
    ...
    try:
        ...
        # Get all queued reservations (NOT pending or preparing - those are handled by message queue and background threads)
        ...
        for status in queued_statuses:
            try:
                # Use PostgreSQL to query by status
                raw_reservations = list_reservations_by_status(status)
                # Filter out reservations that are too new (less than 30 seconds old)
                # This prevents collision with message queue processing
...
def process_cancellation_request(record: dict[str, Any]) -> bool:
    ...
    try:
        ...
        try:
            ...
            if domain_name:
                try:
                    # Mark domain mapping as inactive (delete it)
                    from shared.dns_utils import delete_domain_mapping
                    delete_domain_mapping(domain_name)
                    logger.info(
    ...
    try:
        ...
        if "Jupyter Lab started successfully" in exec_resp:
            ...
            try:
                ...
                domain_name = None
                if reservation_item:
                    domain_name = reservation_item.get("domain_name")
...
def disable_jupyter_in_pod(
    ...
    try:
        ...
        if (
            ...
            update_reservation(reservation_id, updates)
            logger.info(
...
def process_delete_disk_action(record: dict[str, Any]) -> bool:
    ...
    try:
        ...
        # 1. Update database to mark disk as deleted
        try:
            marked_deleted_at = message.get('requested_at', str(int(time.time())))
            updates = {
                'is_deleted': True,
                'delete_date': delete_date,
                'marked_deleted_at': marked_deleted_at
            }
            update_disk(user_id, disk_name, updates)
            logger.info(f"Updated database: marked disk '{disk_name}' as deleted")
...
def process_create_disk_action(record: dict[str, Any]) -> bool:
    ...
    try:
        ...
        # Create disk entry in database
        try:
            now = datetime.now(UTC).isoformat()

            # Create the disk entry (only if it doesn't exist)
            # Check if disk already exists
            existing_disk = get_disk(user_id, disk_name)
            if existing_disk:
            ...
            disk_data = {
                'user_id': user_id,
                'disk_name': disk_name,
                'disk_size': 1024,  # Default 1TB disk (note: column is disk_size not size_gb)
                'created_at': now,
                'last_used': now,
                'snapshot_count': 0,
...
def process_extend_reservation_action(record: dict[str, Any]) -> bool:
    ...
    try:
        ...
        try:
            # Build update for reservation extension
            current_timestamp = int(time.time())
            # Note: The actual update is done below with update_reservation()
            # This section is being refactored
            old_update_expression = "SET expires_at = :new_expires_at, last_updated = :timestamp"
            expression_values = {
            ...
            if "duration_hours" in reservation:
                ...
                # Note: This is handled in the updates dict below
                # expression_values[":new_duration"] = new_duration
            ...
            if "duration_hours" in reservation:
                current_duration = float(reservation.get("duration_hours", 0))
                new_duration = current_duration + float(extension_hours)
                updates["duration_hours"] = new_duration

            update_reservation(full_reservation_id, updates)
            ...
            if domain_name:
                try:
                    # Re-store the domain mapping with updated expiry
                    from shared.dns_utils import store_domain_mapping
                    # Convert ISO string to Unix timestamp for store_domain_mapping
                    from datetime import datetime, UTC
                    new_expiry_dt = datetime.fromisoformat(new_expires_at.replace('Z', '+00:00'))
                    new_expiry_epoch = int(new_expiry_dt.timestamp())
                    
                    # Get current mapping info (we need IP and port)
                    reservation_data = get_reservation(full_reservation_id)
                    if reservation_data:
                        node_ip = reservation_data.get('node_ip')
                        node_port = reservation_data.get('node_port')
                        if node_ip and node_port:
                            store_domain_mapping(domain_name, node_ip, node_port, full_reservation_id, new_expiry_epoch)
                            logger.info(
