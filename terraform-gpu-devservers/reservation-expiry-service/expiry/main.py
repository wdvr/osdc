"""
Reservation Expiry Management CronJob
...
Migrated from Lambda to Kubernetes CronJob
"""
...
from shared.db_pool import get_db_cursor, init_connection_pool, close_connection_pool
from shared.reservation_db import (
    get_reservation, update_reservation, list_reservations_by_status
)
from shared.disk_db import (
    get_disk, update_disk, mark_disk_in_use, get_disks_pending_deletion
)
...
REGION = os.environ.get("AWS_REGION", os.environ.get("REGION", "us-east-2"))

# Global Kubernetes client (reused across execution)
_k8s_client = None
...
def sync_disk_deleted_snapshots() -> int:
    """
    Sync PostgreSQL disk deletion status to EC2 snapshots.
    Tags snapshots with delete-date when disks are marked is_deleted=True in PostgreSQL.
    ...
    try:
        ...
        deleted_disks = get_disks_pending_deletion()

        if not deleted_disks:
            logger.debug("No deleted disks found in PostgreSQL")
            return 0

        logger.info(f"Found {len(deleted_disks)} deleted disks in PostgreSQL")
        ...
        for disk in deleted_disks:
            ...
            try:
                ...
                for snapshot in snapshots:
                    ...
                    try:
                        # Convert delete_date to string if it's a date object
                        if hasattr(delete_date, 'strftime'):
                            delete_date_str = delete_date.strftime('%Y-%m-%d')
                        else:
                            delete_date_str = str(delete_date)
                        
                        # Use last_updated or current time for marked-deleted-at tag
                        marked_deleted_at = disk.get('marked_deleted_at') or disk.get('last_updated')
                        if marked_deleted_at:
                            if hasattr(marked_deleted_at, 'timestamp'):
                                marked_deleted_at_str = str(int(marked_deleted_at.timestamp()))
                            else:
                                marked_deleted_at_str = str(int(time.time()))
                        else:
                            marked_deleted_at_str = str(int(time.time()))
                        ...
                        logger.info(f"Tagged snapshot {snapshot_id} with delete-date: {delete_date_str}")
                        tagged_count += 1
...
def sync_completed_snapshots() -> int:
    """
    Sync completed EC2 snapshots to PostgreSQL.
    ...
    try:
        ...
        logger.info(f"Checking {len(snapshots)} completed snapshots for PostgreSQL sync")

        # Check PostgreSQL for each snapshot to see if it's already been processed
        for snapshot in snapshots:
            snapshot_id = snapshot['SnapshotId']
            tags = {tag['Key']: tag['Value'] for tag in snapshot.get('Tags', [])}
            ...
            try:
                # Check if this snapshot has already been synced to PostgreSQL
                disk_item = get_disk(user_id, disk_name)

                if not disk_item:
                    logger.debug(f"Disk '{disk_name}' not found in PostgreSQL (user: {user_id}), skipping snapshot sync")
                    continue
                ...
                if pending_count != 0 or is_backing_up:
                    logger.info(f"Updating PostgreSQL for completed snapshot {snapshot_id} (disk: {disk_name}, user: {user_id}, pending_count: {pending_count}, is_backing_up: {is_backing_up})")
                ...
                logger.warning(f"Error syncing snapshot {snapshot_id} to PostgreSQL: {disk_error}")
...
def run_expiry_checks():
    """Main expiry logic (formerly in handler function)"""
    try:
        current_time = int(time.time())
        logger.info(
            f"Running reservation expiry and cleanup check at timestamp {current_time} ({datetime.fromtimestamp(current_time, tz=UTC)})"
        )

        # Get all active, preparing, and failed reservations
        try:
            # Get active reservations
            active_reservations = list_reservations_by_status("active", limit=1000)
            if len(active_reservations) >= 1000:
                logger.warning("⚠️ Hit pagination limit for active reservations (1000) - some may not be processed!")
            
            # Get preparing reservations
            preparing_reservations = list_reservations_by_status("preparing", limit=1000)
            if len(preparing_reservations) >= 1000:
                logger.warning("⚠️ Hit pagination limit for preparing reservations (1000) - some may not be processed!")
            ...
            for res in active_reservations:
                ...
                try:
                    if isinstance(expires_at_str, str):
                        ...
                        expires_at = int(expires_at_str.timestamp()) if hasattr(expires_at_str, 'timestamp') else 0
        ...
        try:
            failed_reservations = list_reservations_by_status("failed", limit=1000)
            if len(failed_reservations) >= 1000:
                logger.warning("⚠️ Hit pagination limit for failed reservations (1000) - some may not be processed!")
            ...
            for reservation in failed_reservations:
                ...
                try:
                    ...
                    elif hasattr(failed_at, 'timestamp'):
                        failed_timestamp = int(failed_at.timestamp())
                    else:
                ...
                if not check_pod_exists(pod_name):
                    ...
                    if user_id and disk_name:
                        try:
                            mark_disk_in_use(user_id, disk_name, reservation_id=None, in_use=False)
        ...
        try:
            ...
            for pod in gpu_dev_pods:
                ...
                try:
                    # Look up reservation by prefix using PostgreSQL LIKE query
                    # Include pod_name check for additional safety
                    with get_db_cursor() as cur:
                        cur.execute("""
                            ...
                            WHERE reservation_id LIKE %s || '%%'
                              AND pod_name = %s
                            LIMIT 1
                        """, (reservation_id_prefix, pod_name))
                        result = cur.fetchone()
                        reservation = dict(result) if result else None

                    if not reservation:
                        logger.warning(f"Pod {pod_name} has no corresponding reservation in PostgreSQL (searched prefix: {reservation_id_prefix}) - keeping pod")
                        continue
        ...
        try:
            ...
            for status in expired_statuses:
                reservations = list_reservations_by_status(status, limit=1000)
                if len(reservations) >= 1000:
                    logger.warning(f"⚠️ Hit pagination limit for {status} reservations (1000) - some may not be processed!")
                expired_cancelled_reservations.extend(reservations)
            ...
            for reservation in expired_cancelled_reservations:
                ...
                ended_at = reservation.get("reservation_ended", reservation.get("cancelled_at", ""))
                if not ended_at:
                ...
                try:
                    if isinstance(ended_at, str):
                        ...
                        ended_timestamp = int(ended_at.timestamp())
                    else:
                        ended_timestamp = int(ended_at)

                    if ended_timestamp < expired_cleanup_threshold:
                        continue  # Too old, skip cleanup
        ...
        for status in stale_statuses:
            ...
            if len(reservations) >= 1000:
                logger.warning(f"⚠️ Hit pagination limit for {status} reservations (1000) - some may not be processed!")
            stale_reservations.extend(reservations)
        ...
        for reservation in active_reservations:
            ...
            try:
                if isinstance(expires_at_str, str):
                    ...
                    expires_at = int(expires_at_str.timestamp())
                else:
                    expires_at = 0
            except (ValueError, AttributeError):
                expires_at = 0
                ...
                if pod_name:
                    ...
                    if launched_at:
                        try:
                            ...
                            elif hasattr(launched_at, 'timestamp'):
                                launched_timestamp = int(launched_at.timestamp())
                            else:
                                launched_timestamp = int(launched_at)
        ...
        # Sync disk deletion status from PostgreSQL to EC2 snapshots
        try:
            tagged_snapshot_count = sync_disk_deleted_snapshots()
            logger.info(f"Tagged {tagged_snapshot_count} snapshots for deletion from PostgreSQL sync")
        except Exception as e:
            logger.error(f"Error syncing disk deletion to snapshots: {e}")
            tagged_snapshot_count = 0

        # Sync completed snapshots to PostgreSQL
        try:
            synced_disk_count = sync_completed_snapshots()
            logger.info(f"Synced {synced_disk_count} completed snapshots to PostgreSQL")
        except Exception as e:
...
def find_disk_by_reservation(user_id: str, reservation_id: str) -> str | None:
    ...
    """
    try:
        # Query disks for this user from PostgreSQL
        with get_db_cursor() as cur:
            cur.execute("""
                SELECT disk_name, reservation_id
                FROM disks
                WHERE user_id = %s
                  AND (reservation_id = %s
                       OR reservation_id LIKE %s || '%%')
                LIMIT 1
            """, (user_id, reservation_id, reservation_id[:8]))
            result = cur.fetchone()
...
def handle_oom_event(reservation: dict, oom_info: dict) -> bool:
    ...
    Updates PostgreSQL with OOM tracking information.
    ...
    try:
        ...
        if last_recorded_oom:
            if isinstance(last_recorded_oom, str):
                last_recorded_oom_str = last_recorded_oom
            else:
                last_recorded_oom_str = last_recorded_oom.isoformat() if hasattr(last_recorded_oom, 'isoformat') else str(last_recorded_oom)
            
            if last_recorded_oom_str == new_oom_time:
                logger.debug(f"OOM event already recorded for reservation {reservation_id[:8]}")
                return False

        # Update reservation with OOM info using PostgreSQL
        update_reservation(reservation_id, {
            'last_oom_at': new_oom_time,
            'oom_count': new_oom_count,
            'oom_container': oom_info.get("oom_container", "unknown")
        })
...
def warn_user_expiring(reservation: dict[str, Any], warning_minutes: int) -> None:
    ...
    try:
        ...
        # Update reservation to mark this specific warning as sent using PostgreSQL
        warning_key = f"{warning_minutes}min_warning_sent"
        warnings_sent = reservation.get("warnings_sent", {})
...
def expire_reservation_due_to_missing_pod(reservation: dict[str, Any]) -> None:
    ...
    try:
        ...
        # Update reservation status to expired using PostgreSQL
        now = datetime.now(UTC).isoformat()
        update_reservation(reservation_id, {
            'status': 'expired',
            'reservation_ended': now,
            'failure_reason': 'Pod was manually deleted or removed outside of reservation system'
        })
...
def expire_stuck_preparing_reservation(reservation: dict[str, Any]) -> None:
    ...
    try:
        ...
        # Update reservation status to failed using PostgreSQL
        now = datetime.now(UTC).isoformat()
        update_reservation(reservation_id, {
            'status': 'failed',
            'failed_at': now,
            'reservation_ended': now,
            'failure_reason': 'Reservation stuck in preparing status for more than 1 hour - likely pod creation failed'
        })
...
def expire_reservation(reservation: dict[str, Any]) -> None:
    ...
    try:
        ...
        # 1. Update reservation status to expired using PostgreSQL
        logger.info(
        ...
        try:
            ...
            logger.error(
                f"Failed to update PostgreSQL status for reservation {reservation_id}: {db_error}"
            )
            raise
        ...
        if pod_name:
            ...
            try:
                ...
                # The PostgreSQL status is already updated correctly
        else:
            logger.warning(
...
def cancel_stale_reservation(reservation: dict[str, Any]) -> None:
    ...
    try:
        ...
        # Update reservation status to cancelled using PostgreSQL
        now = datetime.now(UTC).isoformat()
        update_reservation(reservation_id, {
            'status': 'cancelled',
            'cancelled_at': now,
            'reservation_ended': now,
            'failure_reason': 'Stale reservation - exceeded 48 hour queue time'
        })
    ...
    try:
                    ...
                    try:
                        ...
                        waiter.wait(
                            ...
                            WaiterConfig={
                                ...
                                'MaxAttempts': 30  # Wait up to 7.5 minutes (15s * 30 = 450s) - fits in 10-min CronJob timeout
                            }
                        )
                        logger.info(f"Snapshot {snapshot_id} completed successfully")

                        # Step 3.5: Update PostgreSQL to reflect snapshot completion
                        if disk_name:
                            try:
                                ...
                                if snapshot_response.get('Snapshots'):
                                    ...
                                    snapshot_content_s3 = tags.get('snapshot_content_s3')
                                    snapshot_disk_size = tags.get('disk_size')
                                    logger.info(f"Updating PostgreSQL for completed snapshot {snapshot_id} (disk: {disk_name}, size: {size_gb}GB, disk_size: {snapshot_disk_size})")
                                    update_disk_snapshot_completed(user_id, disk_name, size_gb, snapshot_content_s3, snapshot_disk_size)
                                    logger.info(f"Successfully updated PostgreSQL for disk '{disk_name}'")
                            except Exception as update_error:
                                logger.warning(f"Error updating PostgreSQL for snapshot completion: {update_error}")
                                # Don't fail cleanup if PostgreSQL update fails
        ...
        if reservation_data:
            ...
            if final_user_id and final_disk_name:
                try:
                    mark_disk_in_use(final_user_id, final_disk_name, reservation_id=None, in_use=False)
        ...
        if reservation_data:
            ...
            if error_user_id and error_disk_name:
                try:
                    mark_disk_in_use(error_user_id, error_disk_name, reservation_id=None, in_use=False)
...
def main():
    ...
    try:
        ...
        # Run expiry checks (existing logic from handler)
        result = run_expiry_checks()
        
        logger.info(f"Expiry check completed successfully: {result}")
        return 0
    except Exception as e:
        logger.error(f"Expiry check failed: {e}", exc_info=True)
        return 1
    finally:
